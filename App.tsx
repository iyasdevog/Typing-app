...
import { Timer, Zap, Target, BookOpen, Settings, Award, GraduationCap, RefreshCw, Users, Trophy, UserCheck, ShieldCheck, Trash2, Download, Play, Keyboard, Activity } from 'lucide-react';
...
// Optimized Audio Engine (Singleton Pattern)
const AudioEngine = (() => {
  let ctx: AudioContext | null = null;
  const init = () => { if (!ctx) ctx = new (window.AudioContext || (window as any).webkitAudioContext)(); };
...
const App: React.FC = () => {
  ...
  // High-performance alignment engine with look-ahead re-sync
  ...
  const calculateMarks = useCallback((wpm: number, accuracy: number, maxMarks: number, targetWpm: number) => {
    ...
    const speedFactor = Math.min(wpm / targetWpm, 1.15); 
    return Math.min(Math.round((maxMarks * 0.7 * accFactor) + (maxMarks * 0.3 * speedFactor)), maxMarks);
  }, []);
  ...
  const startTest = useCallback(() => {
    if (!student.admissionNumber.trim() || !student.studentName.trim()) {
      alert("Academic credentials required to proceed.");
      return;
    }
    setCountdown(3);
  }, [student]);
  ...
  const handleInput = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    const accuracy = val.length > 0 ? Math.round((alignment.matchCount / Math.max(val.length, alignment.currentTargetIdx)) * 100) : 100;
  ...
  const exportToCSV = () => {
    ...
    a.href = url;
    a.download = `typing_report_class_${student.className}.csv`;
    a.click();
  };
  ...
  return (
    <div className={`min-h-screen bg-[#0a0f1e] text-slate-100 flex flex-col font-sans transition-all duration-700 ${status === TestStatus.RUNNING ? 'bg-[#050811]' : ''}`}>
      <header className={`sticky top-0 z-50 bg-[#0a0f1e]/80 backdrop-blur-xl border-b border-white/5 transition-opacity duration-500 ${status === TestStatus.RUNNING ? 'opacity-20 pointer-events-none' : 'opacity-100'}`}>
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-black tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-white to-slate-400">TypingMaster Pro</h1>
              <p className="text-[10px] text-indigo-400 font-bold uppercase tracking-[0.2em]">Academic Control Console</p>
            </div>
          </div>
        ...
        {activeTab === 'test' ? (
          status === TestStatus.COMPLETED ? (
            ...
            <div className="space-y-12">
              <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 transition-all duration-700 ${status === TestStatus.RUNNING ? 'scale-95 opacity-90' : 'scale-100 opacity-100'}`}>
                <StatsCard label="Timer" value={`${timeLeft}s`} icon={<Timer className="w-6 h-6" />} color="text-indigo-400" />
                <StatsCard label="Net WPM" value={stats.wpm} icon={<Zap className="w-6 h-6" />} color="text-amber-400" pulse={stats.wpm > settings.targetWpm} />
                <StatsCard label="Accuracy" value={`${stats.accuracy}%`} icon={<Target className="w-6 h-6" />} color={stats.accuracy > 90 ? "text-emerald-400" : "text-rose-400"} pulse={stats.accuracy < 90} />
                <StatsCard label="Grade" value={stats.currentMarks} icon={<GraduationCap className="w-6 h-6" />} color="text-violet-400" />
              </div>

              <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 items-start">
                <div className="lg:col-span-2 space-y-8">
                  <div className={`bg-slate-900/40 border border-slate-800 rounded-[2.5rem] p-12 relative overflow-hidden group shadow-2xl transition-all duration-500 ${status === TestStatus.RUNNING ? 'ring-2 ring-indigo-500/30' : ''}`}>
                    {!isFormValid && (
                      <div className="absolute inset-0 z-20 bg-[#0a0f1e]/98 backdrop-blur-xl flex items-center justify-center p-8">
                        <div className="max-w-md w-full bg-slate-900 border border-slate-800 rounded-3xl p-10 shadow-3xl ring-1 ring-white/5">
                          <h3 className="text-2xl font-black mb-8 flex items-center justify-center gap-3"><Users className="w-8 h-8 text-indigo-500" /> Identity Verification</h3>
                          <div className="space-y-6 text-left">
                            <div className="space-y-2">
                              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Admission ID</label>
                              <input type="text" className="w-full bg-slate-950 border border-slate-800 rounded-xl px-5 py-4 outline-none focus:ring-2 ring-indigo-500 transition-all font-mono" placeholder="CS-2024-XXX" value={student.admissionNumber} onChange={e => setStudent({...student, admissionNumber: e.target.value})} />
                            </div>
                            <div className="space-y-2">
                              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Legal Name</label>
                              <input type="text" className="w-full bg-slate-950 border border-slate-800 rounded-xl px-5 py-4 outline-none focus:ring-2 ring-indigo-500 transition-all" placeholder="John Doe" value={student.studentName} onChange={e => setStudent({...student, studentName: e.target.value})} />
                            </div>
                            <div className="space-y-2">
                    ...
                    <div className="relative mb-8">
                      <div className={`flex items-center justify-between mb-10 pb-4 border-b border-white/5 transition-all duration-700 ${status === TestStatus.RUNNING ? 'opacity-30 blur-[1px]' : ''}`}>
                    ...
                    <textarea ref={inputRef} className="absolute inset-0 w-full h-full opacity-0 cursor-default" value={userInput} onChange={handleInput} disabled={status === TestStatus.IDLE || countdown !== null} />
                    
                    {status === TestStatus.IDLE && (
                      <div className="mt-8">
                        <button onClick={startTest} className="w-full bg-indigo-600 hover:bg-indigo-500 py-6 rounded-3xl font-black text-xl flex items-center justify-center gap-4 transition-all shadow-2xl shadow-indigo-600/40 hover:scale-[1.02] active:scale-95">
                          <Play className="w-8 h-8 fill-current" /> INITIALIZE ASSESSMENT
                        </button>
                      </div>
                    )}
                  </div>
                ...
                <div className={`space-y-8 transition-opacity duration-500 ${status === TestStatus.RUNNING ? 'opacity-20 hover:opacity-100' : 'opacity-100'}`}>
                  <div className="bg-slate-900/40 border border-slate-800 rounded-[2.5rem] p-8 shadow-xl">
                    <div className="flex items-center gap-3 mb-8 pb-4 border-b border-white/5">
                      <Settings className="w-5 h-5 text-indigo-400" />
                      <h3 className="font-black text-white uppercase tracking-widest text-xs">Configuration</h3>
                    </div>
                    
                    <div className="space-y-8 text-left">
                      <div className="space-y-3">
                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Subject Module</label>
                      ...
                      <div className="space-y-3">
                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Speed Proficiency Goal</label>
                      ...
                      <div className="pt-6 border-t border-white/5 space-y-4">
                        ...
                        <button onClick={() => resetTest(false)} className="w-full text-slate-600 hover:text-rose-500 text-[10px] font-black uppercase tracking-[0.4em] transition-colors py-2">Purge Session</button>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-gradient-to-br from-indigo-500/10 to-violet-500/10 border border-indigo-500/20 rounded-[2rem] p-8">
                    <div className="flex items-center gap-4 mb-4 text-indigo-400">
                      ...
                      <span className="font-black uppercase tracking-widest text-xs">Live Analytics</span>
                    </div>
          ...
          <div className="animate-in fade-in slide-in-from-bottom-8 duration-1000">
            <div className="flex items-center justify-between mb-12">
              <div className="flex items-center gap-6">
                ...
                <div>
                  <h2 className="text-4xl font-black text-white tracking-tighter">Academic Records</h2>
                  <p className="text-slate-500 text-sm font-bold uppercase tracking-[0.3em] mt-1">Section: Class {student.className}</p>
                </div>
              </div>
              <div className="flex items-center gap-4">
                <button onClick={exportToCSV} className="bg-emerald-600/10 hover:bg-emerald-600/20 text-emerald-500 border border-emerald-500/20 px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-[0.2em] flex items-center gap-3 shadow-lg shadow-emerald-500/5 transition-all active:scale-95">
                  <Download className="w-5 h-5" /> Export DB
                </button>
                <button onClick={() => { if(confirm("Confirm database purge?")) { setLeaderboard([]); localStorage.removeItem('cs_typing_leaderboard'); }}} className="p-4 text-rose-500/40 hover:text-rose-500 transition-colors"><Trash2 className="w-7 h-7" /></button>
              </div>
            </div>
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-slate-900/80 border-b border-slate-800 text-slate-500 text-[10px] uppercase font-black tracking-[0.3em]">
                    ...
                    <th className="px-8 py-8 text-center">Proficiency</th>
                    <th className="px-8 py-8 text-center">Quality</th>
                    <th className="px-12 py-8 text-right">Final Grade</th>
                  </tr>
                </thead>
                  ...
                  {filteredLeaderboard.length > 0 ? filteredLeaderboard.map((e, i) => (
                    <tr key={e.id} className="hover:bg-indigo-500/[0.04] transition-colors group">
                      <td className="px-12 py-8">
                        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black text-xl shadow-lg ${i === 0 ? 'bg-amber-500 text-amber-950 shadow-amber-500/20' : i === 1 ? 'bg-slate-300 text-slate-800' : i === 2 ? 'bg-orange-500 text-orange-950' : 'bg-slate-800 text-slate-500'}`}>{i + 1}</div>
                      </td>
                      ...
                      <td className="px-8 py-8 text-center">
                        <div className="text-2xl font-black text-indigo-400">{e.wpm} <span className="text-[10px] opacity-40">WPM</span></div>
                      </td>
                      <td className="px-8 py-8 text-center">
                        <span className={`px-4 py-2 rounded-full text-[10px] font-black ${e.accuracy > 90 ? 'bg-emerald-500/10 text-emerald-500' : 'bg-rose-500/10 text-rose-500'}`}>{e.accuracy}% ACC</span>
                      </td>
                    ...
                    <tr>
                      <td colSpan={5} className="px-12 py-32 text-center opacity-10">
                        ...
                        <p className="font-black text-xl uppercase tracking-[0.5em]">No Data Cached</p>
                      </td>
                    </tr>
                  )}
                </tbody>
      ...
      <footer className="py-10 border-t border-white/5 text-center transition-opacity duration-500 overflow-hidden">
        <p className="text-[10px] font-black text-slate-700 uppercase tracking-[0.8em]">Optimized Aligned Assessment Interface v3.5 // Performance Cluster Active</p>
      </footer>
    </div>
  );
};
